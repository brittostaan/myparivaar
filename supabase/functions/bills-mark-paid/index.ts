...
interface BillsMarkPaidRequest {
  bill_id: string
  is_paid: boolean
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { bill_id, is_paid }: BillsMarkPaidRequest = await req.json()
    ...
    if (typeof is_paid !== 'boolean') {
      return new Response(JSON.stringify({ error: 'is_paid must be true or false' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    const { data: bill, error: updateError } = await supabase
      ...
      .update({
        is_paid,
        paid_on: is_paid ? new Date().toISOString() : null,
      })
      .eq('id', bill_id)
      .select()
      .single()
    ...
    if (updateError) {
      console.error('bills-mark-paid DB error:', updateError)

...
interface InvestmentDeleteRequest {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { investment_id }: InvestmentDeleteRequest = await req.json()
    ...
    if (deleteError) {
      console.error('investment-delete DB error:', deleteError)
      return new Response(JSON.stringify({ error: 'Failed to delete investment' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }

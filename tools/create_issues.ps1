...
$issues = @(
  @{
    ...
    body   = @"
**File:** ``lib/models/expense.dart`` (~lines 37-47)
...
See Docs/BUG_REPORT.md for full details.
"@
  },
  @{
...
- Creating/updating expenses sends wrong payload — silent server-side errors
...
"@
  ...
  @{
    ...
    labels = "bug,priority: critical"
    body   = @"
**File:** ``lib/models/member.dart`` (~line 39)
...
Every action on the Budget screen fails with 404 Not Found or a CORS error.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: critical"
    body   = @"
**File:** ``lib/widgets/navigation_shell.dart`` (~line 60)

``Navigator.of(context).pushNamed(targetRoute)`` pushes a new route on every tab tap. After visiting multiple tabs, back button walks the full tab history instead of exiting the app.
...
"@
  ...
  @{
    title  = "BUG-006: BuildContext used across async gaps in 5 files — crash after dispose"
...
Context is used after an await without checking ``if (!mounted) return``. If the widget is disposed during the async gap the app crashes with 'Looking up a deactivated widget's ancestor is unsafe'.

**Fix pattern:** Capture services before any await, then guard with ``if (!mounted) return`` after every await.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: high"
    body   = @"
**File:** ``lib/services/email_service.dart`` (~line 112)

Content-Type is set to application/json but no body is provided. Deno's ``request.json()`` throws a parse error, causing sync to always fail.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: high"
    body   = @"
**File:** ``lib/services/auth_service.dart`` (~lines 238-243)

The method matches any error containing 'auth', 'jwt', or 'token' and triggers an automatic signOut(). This is far too broad:
- 'Failed to authenticate proxy' -> signs user out
- 'auth-bootstrap returned 503' -> signs user out

**Impact:** Users randomly logged out on transient network errors.
...
"@
  ...
  @{
    title  = "BUG-010: VoiceExpenseScreen has no registered route — completely unreachable"
    labels = "bug,priority: high"
    body   = @"
...
"@
  ...
  @{
    ...
    body   = @"
**File:** ``lib/services/notification_service.dart`` (~lines 143-172)

monthlyBudgets and currentSpending are hardcoded with fictional values. Budget alerts fire based on made-up numbers regardless of the user's actual data.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: medium"
    body   = @"
**File:** ``lib/services/notification_service.dart`` (~lines 174-182)
...
``final exists = _notifications.any((n) => n.data['billId'] == billId); if (exists) return;``
...
_validateSupabaseConfig() throws a StateError before runApp() is called. There is no Flutter widget tree to display an error message, so the user sees a blank screen.

**Fix:** Wrap the validate call in try/catch and call runApp(MaterialApp(...)) with an error message on failure.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: medium"
    body   = @"
**File:** ``lib/screens/dashboard_screen.dart`` (~lines 57-78)

When expense-stats fails, the fallback refetches ALL expenses (no limit) to compute totalBalance, discarding the already-fetched limit:10 list. Every dashboard load makes 2 API calls.

**Fix:** Reuse the already-fetched expenses list in the fallback — no second API call needed.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: medium"
    body   = @"
**File:** ``lib/widgets/balance_card.dart`` (~line 72)

The card title is hardcoded as 'Expense vs Budget' but the value shown is totalBalance (income minus expenses). This is a misleading label.
...
"@
  ...
  @{
    title  = "BUG-016: EmailSettingsScreen._connectEmailAccount — async gap after showDialog"
    labels = "bug,priority: medium"
    body   = @"
**File:** ``lib/screens/email_settings_screen.dart`` (~lines 75-82)

showDialog is called without await, then Provider.of(context) and getIdToken() are called after an await. If the widget is unmounted this crashes.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: medium"
    body   = @"
**File:** ``lib/models/import_result.dart`` (~lines 14-16)

RowError.fromJson uses hard ``as int`` / ``as String`` casts. Any null/missing field from the Edge Function throws a TypeError, crashing the CSV import preview.
...
"@
  ...
  @{
    ...
    body   = @"
**File:** ``lib/widgets/quick_actions_grid.dart`` (~line 46)

``crossAxisCount: actions.length.clamp(2, 4)`` never allows 1 column. A single action renders in a 2-column grid, leaving an empty cell.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: low"
    body   = @"
**File:** ``lib/main.dart`` (~lines 753-757)

The responsive wrapper always uses Colors.white. When dark mode is enabled this creates a white border mismatch around the constrained viewport.

**Fix:** Use ``Theme.of(context).scaffoldBackgroundColor`` instead.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: low"
    body   = @"
**File:** ``lib/services/voice_service.dart`` (~lines 67-72)
...
"@
  ...
  @{
    ...
    labels = "bug,priority: low"
    body   = @"
**File:** ``lib/models/app_user.dart`` (~line 50)

``supabaseUserId: json['firebase_uid']?.toString() ?? ''``

An empty string silently passes through downstream callers (admin checks, API calls) instead of indicating the value is not set.
...
"@
  ...
  @{
    title  = "BUG-022: Expense date parsing ignores timezone — potential off-by-one-day"
    labels = "bug,priority: low"
    body   = @"
**File:** ``lib/models/expense.dart`` (~line 43)
...
"@
  ...
  @{
    ...
    labels = "bug,priority: low"
    body   = @"
**File:** ``lib/screens/dashboard_screen.dart`` (~lines 38-95)

If the user pulls to refresh while an initial load is in progress, two concurrent calls to _loadDashboardData run. The slower one may win, displaying stale data.

**Fix:** Add an ``_isFetching`` bool guard — skip the new call if one is already running.
...
The /more bottom-nav slot shows only 'Content coming soon...'. AI chat, Voice expense, CSV import, and Notifications screens all exist and work but are not linked from MoreScreen.

**Fix:** Add navigation entries in MoreScreen for all implemented secondary screens.
...
"@
  ...
  @{
    ...
    labels = "bug,priority: low"
    body   = @"
**File:** ``lib/services/auth_service.dart`` (~lines 255-258)

Flutter Web never throws dart:io's SocketException — it throws http.ClientException. The SocketException catch block and the ``import 'dart:io'`` are dead code on the web target.

**Fix:** Remove ``import 'dart:io'`` and the SocketException catch branch, or wrap with ``if (!kIsWeb)``.
